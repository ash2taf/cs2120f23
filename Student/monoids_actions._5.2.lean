...
/-!
AddMonoid.mk.{u}
  {M : Type u} -- rotation for us
  [toAddSemigroup : AddSemigroup M] -- instance of semigroup over M
  [toZero : Zero M] --r0 in our case
  (zero_add : ∀ (a : M), 0 + a = a) --beginning of the explicit args, mostly proofs here
  (add_zero : ∀ (a : M), a + 0 = a)
  (nsmul : ℕ → M → M)
...
-- \bu gives the scalar mult symbol, nsmul comes from monoid typeclass
...
(P : Type u_11)
[AddMonoid G]
extends VAdd : Type (max u_10 u_11) --maximum of the type unverses for the actions and the states
  vadd : G → P → P
  zero_vadd : ∀ (p : P), 0 +ᵥ p = p
  add_vadd : ∀ (g₁ g₂ : G) (p : P), g₁ + g₂ +ᵥ p = g₁ +ᵥ (g₂ +ᵥ p) --allows optimization by adding actions before applying
=

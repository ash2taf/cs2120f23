...
|g, f => fun a => g (f a)
/-! ex: computes whether str len is even, firts get then then check if that is even-/

def sq (n:Nat) : Nat := n^2
def inc (n:Nat) : Nat := Nat.succ n 

#reduce comp Nat Nat Nat inc sq
#eval comp Nat Nat Nat inc sq 5 = inc_after_sq 5

def isEven (n:Nat) : Bool := n%2 = 0
def is_even_len : String → Bool := comp String Nat Bool isEven String.length

/-!Making comp implicit to cut down on args, clean up:
-/
def comp' {α β γ : Type} : (β → γ) → (α → β) → (α → γ)
|g, f => fun a => g (f a)

def is_even_len' := comp' isEven String.length
